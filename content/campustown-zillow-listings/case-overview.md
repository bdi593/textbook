...
| `days_on_zillow`               | float    | How many days the listing has been (or was) live on Zillow.             |
...
3. Build, evaluate, and interpret a regression-based **price-prediction model**
   using `scikit-learn`, including baseline error metrics (MAE, RMSE, R²,
   MAPE) and residual analysis.
4. Use **spaCy** to clean and structure a free-text listing description, extract
   amenity-style features, and combine them with structured tabular features
   to **measurably improve** the price model.
...
```bash
pip install pandas pyarrow numpy plotly scikit-learn spacy
...
| **Part 2a — Baseline Price Predictor**            | 20     | Reproducible `scikit-learn` pipeline (preprocessing + model) trained on a held-out split; reports MAE, RMSE, R², and MAPE; residual analysis breaks errors down by market and price band; chosen model is justified in plain English. |
| **Part 2b — NLP Feature Engineering with spaCy**  | 20     | Uses spaCy (`en_core_web_sm`) to tokenize, lemmatize, and extract structured signals from `description`; engineered features (e.g., amenity flags, TF-IDF, noun-chunk counts) are added to the Part 2a pipeline; the improved model is **measurably better** on the same held-out split, and the improvement is explained. |
...
- Cluster listings (k-means on price-per-sqft, year-built, days-on-market)
  and label the resulting segments — "value plays," "trophy homes,"
  "stale inventory," etc.
